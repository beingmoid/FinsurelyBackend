    ...
    public class SalesInvoiceService : BaseService<SalesInvoice, int>, ISalesInvoiceService
    {
        ...
        protected async override Task WhileUpdating(IEnumerable<SalesInvoice> entities)
        {
            ...
            foreach (var item in entities)
            {
                ...
                .Include(x => x.Transactions).ThenInclude(x => x.LedgarEntries)
                , x => x.Id == item.Id)).SingleOrDefault();
                ////Making Transaction
                var transactionForAgent = sales.Transactions.FirstOrDefault(x=>x.UserDetailId == sales.SalesInvoicePersonId);
                //transaction.Memo = "Opening Balance";
                transactionForAgent.TransactionDate = sales.SalesInvoiceDate;
                transactionForAgent.UserDetailId = sales.SalesInvoicePersonId;
                ...
                LedgarEntries Debitledgar = transactionForAgent.LedgarEntries.SingleOrDefault(x=>x.DebitAccountId!=null);                 Debitledgar.TransactionDate = sales.SalesInvoiceDate;
                ...
                credit_ledger.TransactionDate = sales.SalesInvoiceDate;
                ...
                //taxEntry.TransactionDate = sales.SalesInvoiceDate;
                //taxEntry.CreditAccountId = BuiltinAccounts.VATPayable;
                //taxEntry.Amount = (decimal)sales.SaleLineItem.FirstOrDefault().VAT;
                //transactionForAgent.LedgarEntries.Add(taxEntry);
                var transactionForCompany = sales.Transactions.SingleOrDefault(x=>x.UserDetailId==sales.InsuranceCompanyId);
                ...
                _context.Set<SalesInvoice>().Update(sales).State = EntityState.Modified;
                  await  _context.SaveChangesAsync();
             //await _context.Set<Transaction>().BulkMergeAsync(new[] {transactionForAgent,transactionForCompany});
        ...
        public async Task<bool> UpdateAsync(int id, SalesInvoice entity)
        {
            ...
            var sales =  _context.Set<SalesInvoice>().SingleOrDefault(x => x.Id == id);

            //this.Map(entity, sales);
            var entry =_context.Entry(sales);
